...
## Agent corporativo (Windows 11, Linux, iOS, Android)

O agente cobre os requisitos de endpoint com arquitetura segura:
...
- Receber comando e executar localmente quando permitido
- Reportar status/saída (`POST /tasks/v1/agent/commands/{task_id}/status`)

### Matriz de comportamento por sistema operacional
- **Windows 11 / Linux**: execução local de comandos permitidos (allowlist + timeout).
- **iOS / Android**: agente mobile mantém polling, recebe comando e reporta status; comandos de shell local são bloqueados por segurança/plataforma e devem ser feitos via ações MDM (`lock`, `wipe`, políticas).

### Build targets do agente
```bash
# Linux
GOOS=linux GOARCH=amd64 go build -o dist/uem-agent-linux ./agent
# Windows 11
GOOS=windows GOARCH=amd64 go build -o dist/uem-agent-windows.exe ./agent
# Android
GOOS=android GOARCH=arm64 go build -o dist/uem-agent-android ./agent
# iOS (library/embedding)
GOOS=ios GOARCH=arm64 go build ./agent
```
